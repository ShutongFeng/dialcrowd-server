...
function get_interactive_results(t){
  fetch(serverUrl + '/api/get/result/interactive/' + t.props.session._id)
    ...
    .then(function (json){
      ...
      if (json.survey.length > 0){
        ...
        json.survey.forEach(function(data, index){
          if (data.times.length > 0){
            ...
            if (likert_responses.length > 1 && likert_responses.every( (val, i, arr) => val === arr[0] ) || radio_responses.length > 1 && radio_responses.every( (val, i, arr) => val === arr[0] )) {
        ...
        times.forEach(function(data, index){
          ...
          data.times.forEach(function(data2, index2){
            detail.push({"system": data2.system, "duration": data2.time/1000, "userId": data.userId});
          ...
          results_detail.push({"allsame": "no", 
                                "userId": data.userId,
      ...
      const url = serverUrl + '/api/result/interactive/' + t.props.session._id;
      return getWorkerQuality(url).then(workerQuality => {
        let results = json.response;
        results_detail = results_detail.map(
          result => ({
            ...result,
...
function remove_interactive_results(userId){
  fetch(serverUrl + '/api/delete/result/interactive/' + userId)
    .then(function (response) {
      return response.json();
    })
}
...
class InteractiveQuality extends React.Component {
  handleDelete = (key) => {
    ...
    this.setState({system_survey: dataSource2.filter(item => item.userId !== key)});
    this.setState({overall_survey: dataSource3.filter(item => item.userId !== key)});
    this.setState({times: dataSource4.filter(item => item.userId !== key)});
    this.setState({feedback: dataSource5.filter(item => item.userId !== key)});
  ...
  render() {
    ...
    return <div>
      <p>We calculate the worker's time according to how long they interact with each system. 
